...
import { getChatGptPrompts } from 'apiSdk/chat-gpt-prompts';
import { ChatGptPromptInterface } from 'interfaces/chat-gpt-prompt';
import { getApiKeys } from 'apiSdk/api-keys';
import { ApiKeyInterface } from 'interfaces/api-key';
import { getCustomerInteractions } from 'apiSdk/customer-interactions';
import { CustomerInteractionInterface } from 'interfaces/customer-interaction';

import { ApiKeyListPage as ShowCaseList } from 'pages/api-keys';

function DashboardPage() {
  const { session } = useSession();

  const chatGptPromptStat = useSWR<PaginatedInterface<ChatGptPromptInterface>>(
    () => `/chat-gpt-prompts`,
    () => getChatGptPrompts({ limit: 1, offset: 0 }),
  );

  const apiKeyStat = useSWR<PaginatedInterface<ApiKeyInterface>>(
    () => `/api-keys`,
    () => getApiKeys({ limit: 1, offset: 0 }),
  );

  const customerInteractionStat = useSWR<PaginatedInterface<CustomerInteractionInterface>>(
    () => `/customer-interactions`,
    () => getCustomerInteractions({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Chat Gpt Prompts',
      result: chatGptPromptStat?.data?.totalCount,
      isLoading: chatGptPromptStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Api Keys',
      result: apiKeyStat?.data?.totalCount,
      isLoading: apiKeyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Customer Interactions',
      result: customerInteractionStat?.data?.totalCount,
      isLoading: customerInteractionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`api-keys`} passHref legacyBehavior>
                  <Button
