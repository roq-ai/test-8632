...
import { getApiKeys, deleteApiKeyById } from 'apiSdk/api-keys';
import { ApiKeyInterface } from 'interfaces/api-key';

type ColumnType = ColumnDef<ApiKeyInterface, unknown>;
...
export function ApiKeyListPage(props: ApiKeyListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ApiKeyInterface>>(
    () => `/api-keys?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ApiKeyInterface) => {
    if (hasAccess('api_key', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/api-keys/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/api-keys/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('api_key', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/api-keys/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('api_key', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="api_key" />
        </Flex>

        {hasAccess('api_key', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/api-keys/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
