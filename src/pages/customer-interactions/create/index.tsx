...
import { createCustomerInteraction } from 'apiSdk/customer-interactions';
...
function CustomerInteractionCreatePage() {
  ...
  const handleSubmit = async (values: CustomerInteractionInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createCustomerInteraction(values);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Create Customer Interaction',
              isCurrent: true,
            },
          ]}
        />
