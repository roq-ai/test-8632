...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getCustomerInteractions();
    case 'POST':
      return createCustomerInteraction();
    default:
  ...
  async function getCustomerInteractions() {
    ...
    const response = await prisma.customer_interaction
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'customer_interaction'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createCustomerInteraction() {
    ...
    const data = await prisma.customer_interaction.create({
      data: body,
    });
